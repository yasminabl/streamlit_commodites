...
marquee_html = create_marquee(
    text="Prévisions blé : Anticipez les tendances du marché des commodités !",
    speed=12,
    font_size="26px",
    text_color="#FFFFFF",
    background_color="#000000",
...
with col1:
    ...
    df_wheat_mm.index = pd.date_range(start=current_month_start , periods=len(df_wheat_mm), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions avec un index d'extension
    last_date = df_wheat_mm.index[0]  # Première date historique
    ...
    df_mm = pd.DataFrame(
        moyenne_mobile_simple(df_wheat_mm['value_wheat'], 4, forecast_horizon),
    ...
    df_historical = df_wheat_mm.copy()
    df_historical['MM'] = np.nan

    # Partie prévisions : value_wheat=NaN, valeurs MM
    df_forecast = pd.DataFrame(index=forecast_index)
    df_forecast['value_wheat'] = np.nan
...
with col2:
    ...
    df_wheat_les.index = pd.date_range(start=current_month_start , periods=len(df_wheat_les), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions avec un index d'extension
    last_date = df_wheat_les.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')

    # Lissage exponentiel simple (modifié pour ordre décroissant)
    _, forecasts = lissage_exponentiel_simple(df_wheat_les['value_wheat'].values, alpha, forecast_horizon)

    # Étape 3 : Construire les deux parties du dataframe final
    # Partie historique : uniquement les données brutes historiques
    df_historical = df_wheat_les.copy()
    ...
    df_forecast['value_wheat'] = np.nan  # Aucune donnée brute pour les prévisions
    ...
    st.line_chart(df_final[['value_wheat', 'LES']])
...
with col3:
    ...
    df_wheat_led.index = pd.date_range(start=current_month_start , periods=len(df_wheat_led), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions LED avec un index d'extension
    last_date = df_wheat_led.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')

    # Lissage exponentiel double (LED)
    _, forecasts = lissage_exponentiel_double(df_wheat_led['value_wheat'].values, alpha, beta, forecast_horizon)
    ...
    # Étape 3 : Construire les deux parties du dataframe final
    # Partie historique : uniquement les données brutes historiques
    df_historical = df_wheat_led.copy()
    ...
    st.line_chart(df_final[['value_wheat', 'LED']])
...
with col4:
    ...
    df_wheat_hw.index = pd.date_range(start='2024-09', periods=len(df_wheat_hw), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions Holt-Winters avec un index d'extension
    last_date = df_wheat_hw.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')


    # Validation de la longueur des données par rapport à la saisonnalité
    if len(df_wheat_hw) < season_length:
        ...
        _, forecasts = holt_winters(df_wheat_hw['value_wheat'].values, alpha, beta, gamma, season_length, forecast_horizon)
        ...
        # Étape 3 : Construire les deux parties du dataframe final
        # Partie historique : uniquement les données brutes historiques
        df_historical = df_wheat_hw.copy()
    ...
    st.line_chart(df_final[['value_wheat', 'HW']])
