...
marquee_html = create_marquee(
    text="Prévisions maïs: Anticipez les tendances du marché des commodités !",
    speed=12,
    font_size="26px",
    text_color="#FFFFFF",
    background_color="#000000",
...
with col1:
    ...
    df_corn_mm.index = pd.date_range(start=current_month_start , periods=len(df_corn_mm), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions avec un index d'extension
    last_date = df_corn_mm.index[0]  # Première date historique
    ...
    df_mm = pd.DataFrame(
        moyenne_mobile_simple(df_corn_mm['value_corn'], 4, forecast_horizon),
    ...
    df_historical = df_corn_mm.copy()
    df_historical['MM'] = np.nan

    # Partie prévisions : value_corn=NaN, valeurs MM
    df_forecast = pd.DataFrame(index=forecast_index)
    df_forecast['value_corn'] = np.nan
...
with col2:
    ...
    df_corn_les.index = pd.date_range(start=current_month_start , periods=len(df_corn_les), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions avec un index d'extension
    last_date = df_corn_les.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')

    # Lissage exponentiel simple (modifié pour ordre décroissant)
    _, forecasts = lissage_exponentiel_simple(df_corn_les['value_corn'].values, alpha, forecast_horizon)

    # Étape 3 : Construire les deux parties du dataframe final
    # Partie historique : uniquement les données brutes historiques
    df_historical = df_corn_les.copy()
    ...
    df_forecast['value_corn'] = np.nan  # Aucune donnée brute pour les prévisions
    ...
    st.line_chart(df_final[['value_corn', 'LES']])
...
with col3:
    ...
    df_corn_led.index = pd.date_range(start=current_month_start , periods=len(df_corn_led), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions LED avec un index d'extension
    last_date = df_corn_led.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')

    # Lissage exponentiel double (LED)
    _, forecasts = lissage_exponentiel_double(df_corn_led['value_corn'].values, alpha, beta, forecast_horizon)
    ...
    # Étape 3 : Construire les deux parties du dataframe final
    # Partie historique : uniquement les données brutes historiques
    df_historical = df_corn_led.copy()
    ...
    st.line_chart(df_final[['value_corn', 'LED']])
...
with col4:
    ...
    df_corn_hw.index = pd.date_range(start='2024-09', periods=len(df_corn_hw), freq='-1MS')  # Index mensuel descendant

    # Étape 2 : Calcul des prévisions Holt-Winters avec un index d'extension
    last_date = df_corn_hw.index[0]  # Première date historique (la plus récente)
    forecast_index = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')


    # Validation de la longueur des données par rapport à la saisonnalité
    if len(df_corn_hw) < season_length:
        ...
        _, forecasts = holt_winters(df_corn_hw['value_corn'].values, alpha, beta, gamma, season_length, forecast_horizon)
    ...
    st.line_chart(df_final[['value_corn', 'HW']])
