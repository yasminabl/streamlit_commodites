...
image_path = "/Users/bellilyasmina/Downloads/pngegg-4.png" 
...
with col2 :
    ...
    df_coffee2 = df_coffee_2.fillna(value=np.nan)

# Conversion de l'index en datetime si nécessaire
df_coffee.index = pd.to_datetime(df_coffee.index, errors='coerce')

# Calcul du taux de fermeture mensuel en prenant la dernière valeur
df_coffee_monthly = df_coffee.resample('M').last()
...
st.write('sub_df_coffee')
st.write(sub_df_coffee.index.dtype)
...
st.write('sub_df_coffee_ind')
st.write(sub_df_coffee_ind.index.dtype)
