...
with col2 :
    ...
    st.dataframe(df_sugar.describe())
...
sub_df_sugar.index = pd.to_datetime(sub_df_sugar.index)
sub_df_sugar_ind.index = pd.to_datetime(sub_df_sugar.index)

years = sub_df_sugar.index.year.unique()
months = range(1, 13)  
last_month = sub_df_sugar.index[-1].month 
current_month = sub_df_sugar.index[0].month 
...
sub_df_sugar_filtered = sub_df_sugar[(sub_df_sugar.index >= pd.to_datetime(start_date)) & 
                            (sub_df_sugar.index <= pd.to_datetime(end_date))]

sub_df_sugar_ind_filtered = sub_df_sugar_ind[(sub_df_sugar.index >= pd.to_datetime(start_date)) & 
                            (sub_df_sugar_ind.index <= pd.to_datetime(end_date))]
...
st.line_chart(sub_df_sugar_ind_filtered[columns_to_display])
#_______________________________________________________________________________________________________________________
...
if 'MM' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_sugar_ind_filtered.index, y=sub_df_sugar_ind_filtered['MM_20'], mode='lines', name='MM 20'))
    fig.add_trace(go.Scatter(x=sub_df_sugar_ind_filtered.index, y=sub_df_sugar_ind_filtered['MM_200'], mode='lines', name='MM 200'))
