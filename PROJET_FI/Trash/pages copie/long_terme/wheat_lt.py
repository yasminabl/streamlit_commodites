...
image_path = "/Users/paultournaire/Desktop/Master/M2/Streamlit/PROJET/pages/picture/wheat.jpeg" 
...
df_wheat_2.index = pd.to_datetime(df_wheat.index, errors='coerce')

# Calcul du taux de fermeture mensuel en prenant la dernière valeur

df_wheat_monthly = df_wheat_2.resample('M').last()

# Calcul du pourcentage de variation par rapport à l'année précédente
df_wheat_monthly['Taux_Fermeture'] = df_wheat_monthly['value_wheat']
df_wheat_monthly['Variation_%'] = df_wheat_monthly['Taux_Fermeture'].pct_change(12) * 100

# Calcul de la volatilité mensuelle
volatility_monthly = df_wheat_monthly['Taux_Fermeture'].pct_change().std()
volatility_monthly_display = f"{volatility_monthly:.2f}%"

# Valeurs actuelles pour la carte (vérification de valeurs non manquantes)
current_rate = df_wheat_monthly['Taux_Fermeture'].iloc[-1] if not df_wheat_monthly['Taux_Fermeture'].empty else np.nan
previous_year_rate = df_wheat_monthly['Taux_Fermeture'].shift(12).iloc[-1] if not df_wheat_monthly['Taux_Fermeture'].shift(12).empty else np.nan
...
trend = "Haussiere" if (df_wheat_monthly['Taux_Fermeture'].diff() > 0).iloc[-3:].all() else "Baissiere"
...
with col2 :
    ...
    st.dataframe(df_wheat.describe())
...
sub_df_wheat_ind['MM_20'] = sub_df_wheat_ind['value_wheat'].iloc[::-1].rolling(window=20).mean().iloc[::-1]
sub_df_wheat_ind['MM_200'] = sub_df_wheat_ind['value_wheat'].iloc[::-1].rolling(window=200).mean().iloc[::-1]
sub_df_wheat_ind['RSI'] = compute_rsi(sub_df_wheat_ind['value_wheat'])
sub_df_wheat_ind['Stochastic'] = compute_stochastics(sub_df_wheat_ind['value_wheat'])

#_______________________________________________________________________________________________________________________


sub_df_wheat.index = pd.to_datetime(sub_df_wheat.index)
sub_df_wheat_ind.index = pd.to_datetime(sub_df_wheat.index)

years = sub_df_wheat.index.year.unique()
months = range(1, 13)  
last_month = sub_df_wheat.index[-1].month 
current_month = sub_df_wheat.index[0].month 
...
sub_df_wheat_filtered = sub_df_wheat[(sub_df_wheat.index >= pd.to_datetime(start_date)) & 
                            (sub_df_wheat.index <= pd.to_datetime(end_date))]

sub_df_wheat_ind_filtered = sub_df_wheat_ind[(sub_df_wheat.index >= pd.to_datetime(start_date)) & 
                            (sub_df_wheat_ind.index <= pd.to_datetime(end_date))]

#_______________________________________________________________________________________________________________________
selected_commodities = st.sidebar.radio("Sélectionnez les commodités à afficher", 
                                        ["Wheat", "Index", "Les deux"], 
...
if selected_commodities == "Wheat":
    columns_to_display = ['value_wheat']
elif selected_commodities == "Index":
    columns_to_display = ['value_index']
else:  # "Les deux"
    columns_to_display = ['value_wheat', 'value_index']

st.subheader(f"Graphique des {selected_commodities}")
st.line_chart(sub_df_wheat_ind_filtered[columns_to_display])
#_______________________________________________________________________________________________________________________
...
if 'MM' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_wheat_ind_filtered.index, y=sub_df_wheat_ind_filtered['MM_20'], mode='lines', name='MM 20'))
    fig.add_trace(go.Scatter(x=sub_df_wheat_ind_filtered.index, y=sub_df_wheat_ind_filtered['MM_200'], mode='lines', name='MM 200'))
if 'RSI' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_wheat_ind_filtered.index, y=sub_df_wheat_ind_filtered['RSI'], mode='lines', name='RSI'))
if 'Stochastic' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_wheat_ind_filtered.index, y=sub_df_wheat_ind_filtered['Stochastic'], mode='lines', name='Stochastic'))
