...
image_path = "/Users/paultournaire/Desktop/Master/M2/Streamlit/PROJET/pages/picture/corn.jpeg" 
...
df_corn_2.index = pd.to_datetime(df_corn.index, errors='coerce')

# Calcul du taux de fermeture mensuel en prenant la dernière valeur

df_corn_monthly = df_corn_2.resample('M').last()

# Calcul du pourcentage de variation par rapport à l'année précédente
df_corn_monthly['Taux_Fermeture'] = df_corn_monthly['value_corn']
df_corn_monthly['Variation_%'] = df_corn_monthly['Taux_Fermeture'].pct_change(12) * 100

# Calcul de la volatilité mensuelle
volatility_monthly = df_corn_monthly['Taux_Fermeture'].pct_change().std()
volatility_monthly_display = f"{volatility_monthly:.2f}%"

# Valeurs actuelles pour la carte (vérification de valeurs non manquantes)
current_rate = df_corn_monthly['Taux_Fermeture'].iloc[-1] if not df_corn_monthly['Taux_Fermeture'].empty else np.nan
previous_year_rate = df_corn_monthly['Taux_Fermeture'].shift(12).iloc[-1] if not df_corn_monthly['Taux_Fermeture'].shift(12).empty else np.nan
...
trend = "Haussiere" if (df_corn_monthly['Taux_Fermeture'].diff() > 0).iloc[-3:].all() else "Baissiere"
...
sub_df_corn_ind['MM_20'] = sub_df_corn_ind['value_corn'].iloc[::-1].rolling(window=20).mean().iloc[::-1]
sub_df_corn_ind['MM_200'] = sub_df_corn_ind['value_corn'].iloc[::-1].rolling(window=200).mean().iloc[::-1]
sub_df_corn_ind['RSI'] = compute_rsi(sub_df_corn_ind['value_corn'])
sub_df_corn_ind['Stochastic'] = compute_stochastics(sub_df_corn_ind['value_corn'])

#_______________________________________________________________________________________________________________________


sub_df_corn.index = pd.to_datetime(sub_df_corn.index)
sub_df_corn_ind.index = pd.to_datetime(sub_df_corn.index)

years = sub_df_corn.index.year.unique()
months = range(1, 13)  
last_month = sub_df_corn.index[-1].month 
current_month = sub_df_corn.index[0].month 
...
sub_df_corn_filtered = sub_df_corn[(sub_df_corn.index >= pd.to_datetime(start_date)) & 
                            (sub_df_corn.index <= pd.to_datetime(end_date))]

sub_df_corn_ind_filtered = sub_df_corn_ind[(sub_df_corn.index >= pd.to_datetime(start_date)) & 
                            (sub_df_corn_ind.index <= pd.to_datetime(end_date))]
...
if selected_commodities == "Corn":
    columns_to_display = ['value_corn']
elif selected_commodities == "Index":
    columns_to_display = ['value_index']
else:  # "Les deux"
    columns_to_display = ['value_corn', 'value_index']

st.subheader(f"Graphique des {selected_commodities}")
st.line_chart(sub_df_corn_ind_filtered[columns_to_display])
#_______________________________________________________________________________________________________________________
...
if 'MM' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_corn_ind_filtered.index, y=sub_df_corn_ind_filtered['MM_20'], mode='lines', name='MM 20'))
    fig.add_trace(go.Scatter(x=sub_df_corn_ind_filtered.index, y=sub_df_corn_ind_filtered['MM_200'], mode='lines', name='MM 200'))
if 'RSI' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_corn_ind_filtered.index, y=sub_df_corn_ind_filtered['RSI'], mode='lines', name='RSI'))
if 'Stochastic' in selected_indicators:
    fig.add_trace(go.Scatter(x=sub_df_corn_ind_filtered.index, y=sub_df_corn_ind_filtered['Stochastic'], mode='lines', name='Stochastic'))
