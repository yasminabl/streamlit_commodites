...
image_path = "/Users/paultournaire/Desktop/Master/M2/Streamlit/PROJET/pages/picture/index.jpeg" 
...
df_index_2.index = pd.to_datetime(df_index.index, errors='coerce')

# Calcul du taux de fermeture mensuel en prenant la dernière valeur

df_index_monthly = df_index_2.resample('M').last()

# Calcul du pourcentage de variation par rapport à l'année précédente
df_index_monthly['Taux_Fermeture'] = df_index_monthly['value_index']
df_index_monthly['Variation_%'] = df_index_monthly['Taux_Fermeture'].pct_change(12) * 100

# Calcul de la volatilité mensuelle
volatility_monthly = df_index_monthly['Taux_Fermeture'].pct_change().std()
volatility_monthly_display = f"{volatility_monthly:.2f}%"

# Valeurs actuelles pour la carte (vérification de valeurs non manquantes)
current_rate = df_index_monthly['Taux_Fermeture'].iloc[-1] if not df_index_monthly['Taux_Fermeture'].empty else np.nan
previous_year_rate = df_index_monthly['Taux_Fermeture'].shift(12).iloc[-1] if not df_index_monthly['Taux_Fermeture'].shift(12).empty else np.nan
...
trend = "Haussiere" if (df_index_monthly['Taux_Fermeture'].diff() > 0).iloc[-3:].all() else "Baissiere"
...
with col2 :
    ...
    st.dataframe(df_index.describe())

df_index_ind = df_index.copy()
#_______________________________________________________________________________________________________________________

df_index_ind['MM_20'] = df_index_ind['value_index'].iloc[::-1].rolling(window=20).mean().iloc[::-1]
df_index_ind['MM_200'] = df_index_ind['value_index'].iloc[::-1].rolling(window=200).mean().iloc[::-1]
df_index_ind['RSI'] = compute_rsi(df_index_ind['value_index'])
df_index_ind['Stochastic'] = compute_stochastics(df_index_ind['value_index'])

#_______________________________________________________________________________________________________________________


df_index.index = pd.to_datetime(df_index.index)
df_index_ind.index = pd.to_datetime(df_index.index)


years = df_index.index.year.unique()
months = range(1, 13)  
last_month = df_index.index[-1].month 
...
df_index_filtered = df_index[(df_index.index >= pd.to_datetime(start_date)) & 
                            (df_index.index <= pd.to_datetime(end_date))]

df_index_ind_filtered = df_index_ind[(df_index.index >= pd.to_datetime(start_date)) & 
                            (df_index_ind.index <= pd.to_datetime(end_date))]

#_______________________________________________________________________________________________________________________
selected_commodities = st.sidebar.radio("Sélectionnez les commodités à afficher", 
                                        ["Index"], 
                                        index=0)  # L'index 2 sélectionne "Les deux" par défaut

# Logique pour ajuster les colonnes à afficher en fonction de la sélection
if selected_commodities == "Index":
    columns_to_display = ['value_index']
    
st.subheader(f"Graphique des {selected_commodities}")
st.line_chart(df_index_filtered[columns_to_display])
#_______________________________________________________________________________________________________________________
...
if 'MM' in selected_indicators:
    fig.add_trace(go.Scatter(x=df_index_ind_filtered.index, y=df_index_ind_filtered['MM_20'], mode='lines', name='MM 20'))
    fig.add_trace(go.Scatter(x=df_index_ind_filtered.index, y=df_index_ind_filtered['MM_200'], mode='lines', name='MM 200'))
if 'RSI' in selected_indicators:
    fig.add_trace(go.Scatter(x=df_index_ind_filtered.index, y=df_index_ind_filtered['RSI'], mode='lines', name='RSI'))
if 'Stochastic' in selected_indicators:
    fig.add_trace(go.Scatter(x=df_index_ind_filtered.index, y=df_index_ind_filtered['Stochastic'], mode='lines', name='Stochastic'))
